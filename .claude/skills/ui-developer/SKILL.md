---
name: ui-developer
description: "Use when building or restyling any frontend UI — pages, components, dashboards, admin screens. Enforces deliberate art direction, design tokens, complete interaction states (hover/focus/loading/error/empty), WCAG AA accessibility, and bans generic AI-generated aesthetics (Inter + purple gradients, centered hero + three cards, default Tailwind look)."
---
...
First: Detect an Existing Design System

Before anything else, look for an established design system in the project — theme or token files, shared component libraries, palettes, CLAUDE.md or design docs. If one exists, **extend that system with its own logic**: reuse its tokens, match its materials and typography, and make new screens feel inevitable within it. Radical new art direction applies only to greenfield work or an explicit redesign request. In an existing product, distinctiveness means executing the established identity better — never replacing it page by page.
...
This applies across independent projects — within a single product, coherence with its established system always wins.
...
The scaffold above is a fill-in template: every token you keep must receive a real value — an empty custom property is invalid CSS. Drop tokens you don't need rather than leaving them blank.
...
No emoji as icon substitutes — unless the project's design system deliberately and consistently uses them.
